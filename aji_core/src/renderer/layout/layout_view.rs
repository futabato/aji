...
fn build_layout_tree(
    ...
    // もしノードが作成されなかった場合、DOM ノードの兄弟ノードを使用して LayoutObject の
    // 作成を試みる。LayoutObject が作成されるまで、兄弟ノードをたどり続ける
    while layout_object.is_none() {
        if let Some(n) = target_node {
            ...
            // もし兄弟ノードが存在しない場合、処理するべき DOM ツリーは終了したので、
            // 今まで作成したレイアウトツリーを返す
    ...
    if let Some(n) = target_node {
        ...
        // 子ノードの兄弟ノードを使用して LayoutObject の作成を試みる。LayoutObject が作成されるか
        // たどるべき兄弟ノードがなくなるまで処理を繰り返す
            ...
            loop {
                ...
                if next_sibling.is_none() && original_dom_node.is_some() {
                    original_dom_node = original_dom_node
                        .expect("next sibling shoud exist")
                        .borrow()
                        .next_sibling();
                    continue;
                }
